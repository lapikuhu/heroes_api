...
class MissionCreate(SQLModel):
    ...
    hero_id: int | None = Field(default=None, index=True, title="ID of the hero assigned to the mission", foreign_key="hero.id")

class MissionReadById(SQLModel):
    name: str = Field(index=True, title="Title of the mission", min_length=5)
    difficulty: int = Field(index=True, title="Difficulty level of the mission", ge=1, le=10)
    completed: bool = Field(index=True, title="Is the mission completed?", default=False)
    hero_id: Optional[int] = Field(default=None, index=True, title="ID of the hero assigned to the mission", foreign_key="hero.id")
...
class MissionGetHero(SQLModel):
    hero_id: int = Field(default=None, index=True, title="ID of the hero assigned to the mission", foreign_key="hero.id")
